...
- ### Продукт:
    ...
    - [Пагінація] `limit` потрібен для контролювання кількості товарів які ви отримаєте
    - [Пагінація] `offset` потрібен для контролювання порцій товарів пропустити.
    - **200** - Знайдено товари. Приклад відповіді:
        ```
        [
            {
...
- ### Користувач:
    ...
    - **401** - Немає доступу без авторизації.
    - **500** - Невідома помилка сервера
    ##### 11. **Відправка листа для контакту** (`POST`): Відправляє лист на скриньку підтримки `/user/contact/`
    - Відправляє повідомлення за наданими даними у формі, пошта на яку відправиться лист вказується у .env під назвою `CONTACT_EMAIL`
    - Приклад запиту:
    ```
    {
        "name": "Your_name",
    ...
    ```
    - **200** - Письмо успішно відправлено. Приклад відповідді:
    ```
    {
        "message": "OK"
    }
    ```
    - **400** - Вказані не вірний формат даних. Пояснення у помилці
    - **500** - Не вказані дані з якого email відправляти. Внутрішня невідома помилка
