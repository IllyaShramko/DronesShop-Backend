...
- ### Продукт:
    ...
    - У ендпоінта є 4 `query`-параметри. `popular`, `new`, `limit`, `offset`.
    ...
    - `limit` потрібен для контролювання кількості товарів які ви отримаєте
    - `offset` потрібен для контролювання скільки товарів пропустити.
    - **200** - Знайдено товари. Приклад відповіді:
        ```
        [
            {
